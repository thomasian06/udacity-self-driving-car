...
State BehaviorPlannerFSM::get_closest_waypoint_goal(const State& ego_state, const SharedPtr<cc::Map>& map,
    ...
    waypoint_0 = lookahead_waypoints[n_wp - 1];
...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
    ...
    auto look_ahead_distance = 0.5 * std::pow(velocity_mag, 2.0) / accel_mag;  // <- Fix This
...
State BehaviorPlannerFSM::get_goal(const State& ego_state, SharedPtr<cc::Map> map) {
    ...
    auto goal_wp = get_closest_waypoint_goal(ego_state, map, look_ahead_distance, is_goal_in_junction);
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal, bool& is_goal_in_junction,
                                           string tl_state) {
    ...
    if (_active_maneuver == FOLLOW_LANE) {
        ...
        if (is_goal_in_junction) {
            ...
            // DONE-goal behind the stopping point: put the goal behind the stopping
        ...
        // DONE-maintain the same goal when in DECEL_TO_STOP state: Make sure the
        ...
        // DONE-use distance rather than speed: Use distance rather than speed...
        // if (utils::magnitude(ego_state.velocity) <=
        //     _stop_threshold_speed) {  // -> Fix this
        if (distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE) {
            // DONE-move to STOPPED state: Now that we know we are close or at the
        ...
        // DONE-maintain the same goal when in STOPPED state: Make sure the new goal
        ...
        if (stopped_secs >= _req_stop_time && tl_state.compare("Red") != 0) {
            // DONE-move to FOLLOW_LANE state: What state do we want to move to, when
