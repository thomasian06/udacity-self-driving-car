...
double diff_cost(vector<double> coeff, double duration, std::array<double, 3> goals, std::array<float, 3> sigma,
                 double cost_weight) {
    /*
...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral, const std::vector<State>& obstacles) {
    ...
    for (auto wp : spiral) {
        ...
        for (size_t c = 0; c < n_circles && !collision; ++c) {
            // DONE-Circle placement: Where should the circles be at? The code below
            ...
            for (auto obst : obstacles) {
                ...
                for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
                    auto actor_center_x = obst.location.x + CIRCLE_OFFSETS[c2] * std::cos(actor_yaw);
                    auto actor_center_y = obst.location.y + CIRCLE_OFFSETS[c2] * std::sin(actor_yaw);

                    // DONE-Distance from circles to obstacles/actor: How do you calculate
                    // the distance between the center of each circle and the
                    // obstacle/actor
                    double dist = std::pow(std::pow(circle_center_x - actor_center_x, 2.0) +
                                               std::pow(circle_center_y - actor_center_y, 2.0),
                                           0.5);  // <- Update
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral, State main_goal) {
    ...
    // DONE-distance between last point on spiral and main goal: How do we
    ...
    auto dist = std::sqrt((delta_x * delta_x) + (delta_y * delta_y) + (delta_z * delta_z));
