...
void VelocityProfileGenerator::setup(const double& time_gap, const double& a_max, const double& slow_speed) {
    _time_gap = time_gap;
    _a_max = a_max;
...
std::vector<TrajectoryPoint> VelocityProfileGenerator::generate_trajectory(const std::vector<PathPoint>& spiral,
    ...
    if (trajectory.size() > 1) {
        ...
        interpolated_state.path_point.x =
            (trajectory[1].path_point.x - trajectory[0].path_point.x) * 0.1 + trajectory[0].path_point.x;
        interpolated_state.path_point.y =
            (trajectory[1].path_point.y - trajectory[0].path_point.y) * 0.1 + trajectory[0].path_point.y;
        interpolated_state.path_point.z =
            (trajectory[1].path_point.z - trajectory[0].path_point.z) * 0.1 + trajectory[0].path_point.z;
        interpolated_state.v = (trajectory[1].v - trajectory[0].v) * 0.1 + trajectory[0].v;
...
std::vector<TrajectoryPoint> VelocityProfileGenerator::decelerate_trajectory(const std::vector<PathPoint>& spiral,
                                                                             const double& start_speed) const {
    std::vector<TrajectoryPoint> trajectory;
...
std::vector<TrajectoryPoint> VelocityProfileGenerator::follow_trajectory(const std::vector<PathPoint>& spiral,
                                                                         const double& start_speed,
                                                                         const double& desired_speed,
                                                                         const State& lead_car_state) const {
    std::vector<TrajectoryPoint> trajectory;
    return trajectory;
}
...
// or Cruise Control
std::vector<TrajectoryPoint> VelocityProfileGenerator::nominal_trajectory(const std::vector<PathPoint>& spiral,
                                                                          const double& start_speed,
    ...
    while (ramp_end_index < (spiral.size() - 1) && (distance < accel_distance)) {
        distance += utils::distance(spiral[ramp_end_index], spiral[ramp_end_index + 1]);
        ramp_end_index += 1;
    }
...
double VelocityProfileGenerator::calc_distance(const double& v_i, const double& v_f, const double& a) const {
    double d{0.0};
    if (std::abs(a) < DBL_EPSILON) {
        d = std::numeric_limits<double>::infinity();
    } else {
        // DONE-calc distance: use one of the common rectilinear accelerated
        ...
        d = 0.5 * (v_f * v_f - v_i * v_i) / a;  // <- Update
    }
    return d;
}
...
a - the acceleration in m / s ^ 2.
*/
double VelocityProfileGenerator::calc_final_speed(const double& v_i, const double& a, const double& d) const {
    double v_f{0.0};
    // DONE-calc final speed: Calculate the final distance. HINT: look at the
    ...
    if (disc <= 0.0) {
        v_f = 0.0;
    } else if (disc == std::numeric_limits<double>::infinity() || std::isnan(disc)) {
