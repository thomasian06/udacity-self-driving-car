...
State MotionPlanner::get_goal_state_in_ego_frame(const State& ego_state, const State& goal_state) {
    ...
    */
    ...
    goal_state_ego_frame.location.x =
        cos_theta * goal_state_ego_frame.location.x - sin_theta * goal_state_ego_frame.location.y;
    goal_state_ego_frame.location.y =
        sin_theta * goal_state_ego_frame.location.x + cos_theta * goal_state_ego_frame.location.y;
    ...
    // works.
    goal_state_ego_frame.rotation.yaw = utils::keep_angle_range_rad(goal_state_ego_frame.rotation.yaw, -M_PI, M_PI);
...
std::vector<State> MotionPlanner::generate_offset_goals_ego_frame(const State& ego_state, const State& goal_state) {
    // Let's transform the "main" goal (goal state) into ego reference frame
    auto goal_state_ego_frame = get_goal_state_in_ego_frame(ego_state, goal_state);

    return generate_offset_goals(goal_state_ego_frame);
}

std::vector<State> MotionPlanner::generate_offset_goals_global_frame(const State& goal_state) {
...
std::vector<State> MotionPlanner::generate_offset_goals(const State& goal_state) {
    ...
    auto yaw_plus_90 = goal_state.rotation.yaw + M_PI_2;  // <- Fix This
    ...
    for (int i = 0; i < _num_paths; ++i) {
        ...
        // DONE-offset goal location: calculate the x and y position of the offset
...
std::vector<int> MotionPlanner::get_best_spiral_idx(const std::vector<std::vector<PathPoint>>& spirals,
...
std::vector<std::vector<PathPoint>> MotionPlanner::transform_spirals_to_global_frame(
    const std::vector<std::vector<PathPoint>>& spirals, const State& ego_state) {
    ...
    for (auto spiral : spirals) {
        ...
        for (auto path_point : spiral) {
            ...
            new_path_point.x = ego_state.location.x + path_point.x * std::cos(ego_state.rotation.yaw) -
                               path_point.y * std::sin(ego_state.rotation.yaw);
            new_path_point.y = ego_state.location.y + path_point.x * std::sin(ego_state.rotation.yaw) +
...
std::vector<std::vector<PathPoint>> MotionPlanner::generate_spirals(const State& ego_state,
...
bool MotionPlanner::valid_spiral(const std::vector<PathPoint>& spiral, const State& offset_goal) {
    auto n = spiral.size();
...
float MotionPlanner::calculate_cost(const std::vector<PathPoint>& spiral, const std::vector<State>& obstacles,
                                    const State& goal) {
